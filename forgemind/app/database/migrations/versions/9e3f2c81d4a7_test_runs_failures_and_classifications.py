...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["task_id"],
            ["tasks.id"],
            name=op.f("fk_test_runs_task_id_tasks"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["worktree_id"],
            ["worktrees.id"],
            name=op.f("fk_test_runs_worktree_id_worktrees"),
            ondelete="SET NULL",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_test_runs")),
    )
    op.create_index(
        op.f("ix_test_runs_task_id"), "test_runs", ["task_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["test_run_id"],
            ["test_runs.id"],
            name=op.f("fk_failures_test_run_id_test_runs"),
            ondelete="CASCADE",
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["task_id"],
            ["tasks.id"],
            name=op.f("fk_failure_classifications_task_id_tasks"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["test_run_id"],
            ["test_runs.id"],
            name=op.f("fk_failure_classifications_test_run_id_test_runs"),
            ondelete="SET NULL",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_failure_classifications")),
    )
    op.create_index(
        op.f("ix_failure_classifications_task_id"),
        "failure_classifications",
        ["task_id"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(
        op.f("ix_failure_classifications_task_id"), table_name="failure_classifications"
    )
