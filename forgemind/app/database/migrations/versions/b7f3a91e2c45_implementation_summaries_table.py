...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["step_id"],
            ["plan_steps.id"],
            name=op.f("fk_implementation_summaries_step_id_plan_steps"),
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["task_id"],
            ["tasks.id"],
            name=op.f("fk_implementation_summaries_task_id_tasks"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["worktree_id"],
            ["worktrees.id"],
            name=op.f("fk_implementation_summaries_worktree_id_worktrees"),
            ondelete="SET NULL",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_implementation_summaries")),
    )
    op.create_index(
        op.f("ix_implementation_summaries_task_id"),
        "implementation_summaries",
        ["task_id"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(
        op.f("ix_implementation_summaries_task_id"),
        table_name="implementation_summaries",
    )
