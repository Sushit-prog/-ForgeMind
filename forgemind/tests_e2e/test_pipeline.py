...
def valid_payload(source_repo) -> dict:
    ...
    fork_url points at the operator's FORK (Phase 10)."""
    return {
...
def test_full_pipeline_worker_driven_to_completed(
...
def test_cancelled_task_stays_failed_under_worker(
    client, db_session, source_repo
) -> None:
...
def test_crash_after_commit_is_healed_by_restart(
    client, db_session, source_repo
) -> None:
    ...
    try:
        ...
        assert wait_for(at_planning, timeout=30), (
            "worker never applied first transition"
        )
    finally:
        proc.terminate()
        proc.wait(timeout=10)
...
def test_kill_between_transitions_resumes_cleanly(
    client, db_session, source_repo
) -> None:
